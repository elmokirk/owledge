...
**Evidence basis:** `tools/owledge.py` (2540 lines), `tools/agent_memory_cli.py`,
...
(`agent-memory-principles`, `agent-memory-runtime-bridge`,
...
HOST_TOOL_FILES + HOST_SKILL_DIRS + `templates/agent-memory/**` + `skills/**`.
...
`agent-memory/exports/upgrade-pending.patch`. **No writes.**"* Manual mode is
...
`agent-memory/exports/upgrade-pending.patch` -> `error: No valid patches in
...
`concept_audit` (`agent_memory_cli.py:5962`) returns findings but asserts
...
suggested_actions}`. Actual (`agent_memory_cli.py:6006-6014`): per-dimension
...
`git status` shows 9 modified files under `internal/agent-memory/exports/` and
3 untracked files under `internal/agent-memory/exports/rag/`. **Fix:** commit
...
### P2-17. `dogfood_sync_check` compares ALL files under `templates/agent-memory/templates/`, not just `*-template.md`.

Code (`agent_memory_cli.py:5455`) uses `source_dir.rglob("*")`. In practice
...
Plan (`:255`): `templates/agent-memory/decisions/concept-audit-YYYY-MM-DD.template.md`.
Actual: `templates/agent-memory/templates/concept-audit-template.md`. The
...
| P0-5 | P0 | `agent_memory_cli.py:5962-6014` + grep `>=4`: 0 hits | META test not enforced | Fix in Phase 3 |
...
| P1-13 | P1 | `agent_memory_cli.py:6006-6014` | No top-level `score` | Fix in Phase 2 |
...
- **Evidence:** this report + `docs/v0.6.1-fix-up-plan.md` + (after fix-up)
  regenerated `internal/agent-memory/exports/finalization-gates/latest.md`

...
MANIFEST_FIELDS = {"checkpoint", "gate_result", "artifacts"}
CHECKPOINT_FIELDS = {"ticket_id", "tested_commit", "input_hash", "next_action", "command_results", "side_effects", "idempotency"}
GATE_FIELDS = {"gate_id", "tested_commit", "input_hash", "passed", "thresholds", "evidence_refs", "reviewer", "limitations"}
...
def validate_checkpoint(value: Any) -> list[str]:
    ...
    errors.extend(f"checkpoint.unknown_field:{key}" for key in sorted(set(value) - CHECKPOINT_FIELDS))
    ...
    elif any(not isinstance(row, dict) or set(row) - {"command", "exit_code"} or not isinstance(row.get("command"), str) or not isinstance(row.get("exit_code"), int) for row in results): errors.append("checkpoint.command_result")
...
def validate_gate_result(value: Any, *, owner_actor: str | None = None) -> list[str]:
    ...
    if not isinstance(value, dict): return errors
    errors.extend(f"gate.unknown_field:{key}" for key in sorted(set(value) - GATE_FIELDS))
...
def validate_evidence_manifest(value: Any, *, expected_commit: str | None = None, expected_input_hash: str | None = None, owner_actor: str | None = None) -> list[str]:
    ...
    if not isinstance(value, dict): return errors
    errors.extend(f"evidence.unknown_field:{key}" for key in sorted(set(value) - MANIFEST_FIELDS))
    if not expected_commit: errors.append("evidence.expected_commit_required")
    if not expected_input_hash: errors.append("evidence.expected_input_hash_required")
    if not owner_actor: errors.append("evidence.owner_actor_required")

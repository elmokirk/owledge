...
Host project = OWLEDGE.md, DESIGN.md, .owledge/
...
full `plugins/owledge-cowork/` folder into the plugin directory that the
runtime reads from.
...
2. Keep `OWLEDGE.md`, `.owledge/`, and `tools/owledge_core.py`
   in that project.
...
Use `owledge-principles` as the primary entrypoint when an agent should
...
```bash
python plugins/owledge-cowork/scripts/capture-claude-event.py < plugins/owledge-cowork/tests/fixtures/session-start.json
python plugins/owledge-cowork/scripts/close-runtime-session.py < plugins/owledge-cowork/tests/fixtures/stop.json

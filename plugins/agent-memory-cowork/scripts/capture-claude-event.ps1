...
function Write-AgentMemoryPluginError {
  ...
  try {
    ...
    # Capture hooks are fail-soft by design.
  }
}
...
function Find-AgentMemoryRoot {
  ...
  while ($true) {
    if ((Test-Path -LiteralPath (Join-Path $current "PROJECT_CONTEXT.md")) -and (Test-Path -LiteralPath (Join-Path $current "agent-memory"))) {
      return $current
    }
...
try {
  ...
  if ($payload.Length -gt 1048576 -and $captureMode -ne "full-private") {
    $payload = "{""hook_event_name"":""PayloadTooLarge"",""payload_hash"":""$([Convert]::ToHexString([System.Security.Cryptography.SHA256]::HashData([System.Text.Encoding]::UTF8.GetBytes($payload))).ToLower())"",""original_chars"":$($payload.Length)}"
  }

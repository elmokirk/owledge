...
| --- | --- | --- | --- | --- |
| T1 Review workflow tooling | `tools/agent_memory_cli.py`, `tools/run-review-workflow.ps1` | Generated artifact quality, wrapper behavior, placeholder replacement | CLI can create concrete review artifacts from all reusable templates without overwriting files | Python compile, wrapper invocation, no unresolved tenant/customer/project placeholders, JSON output includes paths and QA commands |

...
| `python tools/owledge.py research-recall --project-root . --query "tool compatibility"` | No | Deterministically reuse authorized Research Memory before proposing refresh |
| `python tools/owledge.py rag-projection-v1 --project-root .` | Yes, reviewed projection | Build the v0.8 reviewed retrieval projection without exporting raw frontmatter as embeddings |
| `python tools/owledge.py small-model-validate --project-root . --profile 8k --capsule-chars <n> --tools <csv> --hops <n> --retry-count <n> --output-json <path>` | No | Validate a bounded small-model result against profile, retry, tool, and hop constraints |
| `python tools/owledge.py resume-context-v1 --project-root . --state-path <state> --control-path <path> --runtime-resume-model persisted` | No | Emit a bounded, hash-aware resume capsule without loading unchanged control content |
| `python tools/owledge.py gate-sidecar-v1 --project-root . --payload-json <path> --sidecar-path <path>` | Yes, sidecar | Persist and reconstruct the full gate payload behind a capped triage summary |
| `python tools/owledge.py resume-acceptance-v1 --project-root . --state-path <state> --control-path <path> --handoff-path <path> --alignment-state-path <path> --sidecar-path <path>` | Yes, evidence | Run the deterministic v0.8 resume acceptance witness, including stale-handoff and alignment-stop checks |

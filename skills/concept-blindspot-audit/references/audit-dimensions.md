...
`tools/agent_memory_cli.py`). Dimensions 5-8 are guided: the function emits a
...
`source_vs_target_audit` audited `templates/agent-memory/` inside the repo, not
...
- `internal/agent-memory/templates/` mirrors
  `templates/agent-memory/templates/` (covered by `dogfood_sync_check`).
...
`internal/agent-memory/templates/` against
`templates/agent-memory/templates/`. Edits to product templates did not
...
to `internal/agent-memory/templates/`."
...
1. If `global-link.json` exists, does the linked path resolve and is it
   readable? Run `python tools/agent_memory_cli.py --project-root . doctor` and
   inspect the `global-link` check.
2. Are the never-touch files (`PROJECT_CONTEXT.md`, `AGENTS.md`, `CLAUDE.md`,
...
3. List synonyms (e.g. "kit" vs "Agent Memory Kit" vs "Owledge"). Are they
   used consistently or do they drift?
4. Check version strings: `VERSION`, `templates/agent-memory/README.md`
   version, `PROJECT_CONTEXT.template.md` version, plugin `VERSION` files. Do
   they agree?
...
`templates/agent-memory/README.md`=0.4.0,
`PROJECT_CONTEXT.template.md`=0.1.0 — four independent version strings. This

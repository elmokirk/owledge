...
function memoryMarkdownFiles(root) {
  const seen = new Set();
...
function validateMetadata(meta, rel, schema) {
  const issues = [];
...
function loadRecords(root, schema, strict) {
...
function resolveInputPath(root, candidate) {
...
function runFixture(root, fixturePath, records, defaultLimit) {
  const fixture = JSON.parse(fs.readFileSync(resolveInputPath(root, fixturePath), "utf8"));
